...
import { Target, CheckCircle, Circle, Loader2, Sparkles, RotateCcw, Star, TrendingUp, Clock, BookOpen, Download, Zap } from "lucide-react";
...
export function Roadmap() {
  ...
  const [aiSource, setAiSource] = useState(""); // "gemini" | "server"
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (hasKey) {
        try {
          toast.loading("Generating with Gemini AI...", { id: "gen" });
          data = await generateRoadmapWithGemini(skills, weeks, companyType);
          if (data) {
            setAiSource("gemini");
            toast.success("Roadmap generated by Gemini AI ✨", { id: "gen" });
          }
        } catch (geminiErr) {
          toast.dismiss("gen");
          console.warn("Gemini failed, falling back to server:", geminiErr.message);
          toast("Gemini unavailable — using smart fallback", { icon: "⚡", id: "gen" });
          data = null;
        }
      }
      ...
      if (!data) {
        if (!hasKey) toast.loading("Generating roadmap...", { id: "gen" });
        const res = await api.post("/data", { skills, weeks });
        data = res.data;
        setAiSource("server");
        toast.success("Roadmap generated!", { id: "gen" });
      }

      setPlan({ ...data, selectedSkills: skills });
      setCheckedItems({});
      setExpandedWeek(0);
        ...
        {plan && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="flex items-center gap-2">
              {aiSource === "gemini" ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-accent-purple/15 text-accent-purple border border-accent-purple/30">
                  <Zap className="w-3 h-3" /> Generated by Gemini AI
                </span>
              ) : aiSource === "server" ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-surface-3 text-text-muted border border-border">
                  ⚡ Generated by smart fallback
                </span>
              ) : null}
            </div>
